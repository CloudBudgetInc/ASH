({
	fetchContacts: function(cmp) {
		const action = cmp.get("c.getDHContactJunctions");
    action.setStorable();
	...
	createFilters: function(cmp, contactList) {
		...
		const programs = contactList.map(function(contact) {
			if (contact.Data_Hub_Contributor__r && contact.Data_Hub_Contributor__r.Data_Hub_Program__r
				&& contact.Data_Hub_Contributor__r.Data_Hub_Program__r.Name) {
				return contact.Data_Hub_Contributor__r.Data_Hub_Program__r.Name;
			}
			return '';
		}).filter(function(item, index, arr) {
			return item && arr.indexOf(item) === index;
		}).sort();
		cmp.set('v.programs', programs);
		...
		const dataHubs = contactList.map(function(contact) {
			if (contact.Data_Hub_Contributor__r && contact.Data_Hub_Contributor__r.Name) {
				return contact.Data_Hub_Contributor__r.Name;
			}
			return '';
		}).filter(function(item, index, arr) {
			return item && arr.indexOf(item) === index;
		}).sort();
		cmp.set('v.dataHubs', dataHubs);
	},
		...
		const filtered = contacts.filter(function(contact) {
			return Object.keys(this).every((key) => {
				if (key == 'Name') {
					...
					if (contact.Data_Hub_Contributor__r && contact.Data_Hub_Contributor__r.Data_Hub_Program__r) {
						return contact.Data_Hub_Contributor__r.Data_Hub_Program__r.Name === this[key];
					}
					return false;
				} else if (key == 'State') {
					if (contact.Address__r && contact.Address__r.State__c) {
					...
					if (contact.Data_Hub_Contributor__r && contact.Data_Hub_Contributor__r.Name) {
						return contact.Data_Hub_Contributor__r.Name === this[key];
					}
				}
			});
		}, filter);
	...
	addActiveFilter: function(cmp) {
		window.setTimeout(
			$A.getCallback(function() {
				if (!$A.util.hasClass(cmp, 'member-directory__filter-btn--active')) {
					$A.util.addClass(cmp, 'member-directory__filter-btn--active');
				}
		}), 200);
	},
	removeActiveFilter: function(cmp) {
		window.setTimeout(
			$A.getCallback(function() {
				if ($A.util.hasClass(cmp, 'member-directory__filter-btn--active')) {
					$A.util.removeClass(cmp, 'member-directory__filter-btn--active');
				}
		}), 200);
	},
	...
	filterSearch: function(contacts, txt) {
		...
		const filtered = contacts.filter(function(contact) {
			...
			if (contact.Data_Hub_Contributor__r && contact.Data_Hub_Contributor__r.Data_Hub_Program__r) {
				consortium = contact.Data_Hub_Contributor__r.Data_Hub_Program__r.Name.toLowerCase();
			}
			if (contact.Address__r && contact.Address__r.State__c) {
				state = contact.Address__r.State__c.toLowerCase();
			}
			if (contact.Data_Hub_Contributor__r && contact.Data_Hub_Contributor__r.Name) {
				ctu = contact.Data_Hub_Contributor__r.Name.toLowerCase();
			}
