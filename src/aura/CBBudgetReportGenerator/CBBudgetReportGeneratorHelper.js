({
	...
	helpRefreshData: function (cmp) {
		try {
			if(!cmp.get('v.isConsolidated')){
				...
				if(cmp.get('v.isConsolidated')){
					this.helpDownloadExcelConsolidated(cmp, incomeRows, expenseRows);
				}
			}
		} catch (e) {
	...
	helpValidateDataAndSuggestDownloadExcel: function(cmp, incomeRows, expenseRows) {
		if((incomeRows.length + expenseRows.length) > 200){
			if(confirm('The amount of data to display exceeds 200 rows, which may affect performance, we recommend downloading the report as an excel file. Click OK to download the report in excel file. Click Cancel to display the report in a table on the page.')){
				if(confirm('Press OK to download Consolidated Report. Press Cancel to download Split by Sheet Report.')){
					this.helpDownloadExcelConsolidated(cmp, incomeRows, expenseRows);
				}else{
	...
	helpGetRootApp: function (cmp) {
		...
		action.setCallback(this, function (response) {
			...
			if (state === "SUCCESS") {
				...
				const selectedGroup = _isInvalid(apps[0].cb4__Tag2__r.cb4__Text4__c) ? 'accTag5' : apps[0].cb4__Tag2__r.cb4__Text4__c;
	...
	helpGetSectionData: function (cmp, childrenId, type) {
		try {
			...
			let action = cmp.get("c.getBudgetSummaryRowsServer");
			action.setParams({
				"appId": cmp.get("v.recordId"),
	...
	helpDownloadExcelConsolidated: function (cmp, income, expense) {
		...
		try {
			...
			workbook.xlsx.writeBuffer().then(buffer => {
				saveAs(new Blob([buffer]), cmp.get('v.rootApp.cb4__TagLabel__c') + ' Consolidated' + '.xlsx');
				this.helpBackToMainTable(cmp);
			}).catch(err => alert('Error writing excel export' + err));
		} catch (e) {
			alert(e)
		}
	},
